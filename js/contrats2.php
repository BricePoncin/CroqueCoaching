...
<div style="display: block;float: left; margin-left: 20%; width:80%;">
		<form name="frm_filtres" action="index.php?pg=contrats" method="POST">
				<div>
					Pr&eacute;f&eacute;rences : <a href="#" id="ouv_ferm_pref" onclick="javascript:ouvre_ferme('ouv_ferm_pref','preferences');">(+)</a>
								&nbsp; Contrats &agrave; r&eacute;aliser entre :&nbsp;<select id="fuseau_min" name="fuseau_min"><?PHP for($i=0; $i<=24; $i++) {  $selected=($fuseau_min==$i)?" selected=\"selected\"":""; echo "<option value=\"$i\"$selected>H+$i</option>"; } ?></select>
											et  <select id="fuseau_max" name="fuseau_max"><?PHP for($i=0; $i<=24; $i++) {  $selected=($fuseau_max==$i)?" selected=\"selected\"":""; echo "<option value=\"$i\"$selected>H+$i</option>"; } ?></select> <span style="margin: 10px 0; border:1px;">
										...
										<br/><form name="frm_affich" id="frm_affich" action="SELF" method="POST">
												<input type="radio" name="affich" value="ville_a_points"    OnClick="javascript:refresh('ville_a_points');" checked="checked">Afficher les villes intoxiquées &nbsp; 
												<input type="radio" name="affich" value="ville_sans_point"  OnClick="javascript:refresh('ville_sans_point');">Masquer les villes intoxiquées &nbsp;
										</form>
	...
	<div style="width: 100%; display: block; float: left; border: 1; margin: 2px;">
		<table cellpadding="0" cellspacing="0" border="0" width="100%" id="aide_contrats" > 
			<thead>
				<tr>
					<th class="data"><img src="images/city_default.png" /></th>
					<th data-stype="num" class="data"><img src="images/miniMoney.gif" /></th>
					<th class="data"><img src="images/time.gif" /></th>
					<th class="data"><img src="images/finferno.png" /></th>
